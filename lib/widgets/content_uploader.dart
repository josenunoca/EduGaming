...
class _ContentUploaderState extends State<ContentUploader> {
  ...
  Future<void> _pickFile() async {
    ...
    try {
      FilePickerResult? result = await FilePicker.platform.pickFiles(
        ...
        allowedExtensions: ['jpg', 'png', 'mp4', 'mp3', 'pdf', 'xlsx', 'csv', 'pptx'],
      );
  ...
  Future<void> _handleDrop(DropDoneDetails details) async {
    ...
    try {
      ...
      final allowed = ['jpg', 'png', 'mp4', 'mp3', 'pdf', 'xlsx', 'csv', 'pptx'];
    ...
    showDialog(
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setStateDialog) => AlertDialog(
          ...
          title: const Text('Configurar Conteúdo', style: TextStyle(color: Colors.white)),
          content: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              Text('Ficheiro: ${file.name}', style: const TextStyle(color: Colors.white70, fontSize: 12)),
              ...
              DropdownButtonFormField<String>(
                ...
                items: const [
                  DropdownMenuItem(value: 'support', child: Text('Material de Apoio')),
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: _isLoading ? null : () async {
                if (file.bytes == null) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(content: Text('Erro: Ficheiro inválido (sem conteúdo)')),
                  );
                  return;
                }
                ...
                try {
                  ...
                  final downloadUrl = await fs.uploadContentFile(file.bytes!, file.name);
                  ...
                  else if (['mp3', 'wav', 'aac'].contains(ext)) type = 'audio';
                  else if (['xlsx', 'csv', 'xls'].contains(ext)) type = 'spreadsheet';
                  ...
                  if (mounted) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(content: Text('Erro no upload. Tente novamente.')),
                    );
                  }
                } finally {
