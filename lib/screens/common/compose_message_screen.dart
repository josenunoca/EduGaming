...
class _ComposeMessageScreenState extends State<ComposeMessageScreen> {
  ...
  Future<void> _fetchPotentialRecipients() async {
    ...
    final currentUserData = await service.getUserData(service.currentUser?.uid ?? '');
  ...
  void _sendMessage() async {
    if (_selectedRecipientIds.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: AiTranslatedText('Por favor, selecione pelo menos um destinatário.')),
      );
      return;
    }
    if (_subjectController.text.isEmpty || _bodyController.text.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: AiTranslatedText('Por favor, preencha o assunto e a mensagem.')),
      );
      return;
    }
    ...
    if (mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: AiTranslatedText('Mensagem enviada com sucesso!')),
      );
    }
  }
            ...
            children: [
              ...
              if (!_showCc)
                TextButton(
                  ...
                  child: const AiTranslatedText('+ Adicionar Cc', style: TextStyle(color: Color(0xFF00D1FF))),
                )
              else
  ...
  Widget _buildRecipientField(String label, List<String> selectedIds, {required bool isCc}) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        ...
        Wrap(
          ...
          children: [
            ...selectedIds.map((id) {
              final user = _potentialRecipients.firstWhere(
                (u) => u.id == id, 
                orElse: () => UserModel(id: id, name: '...', email: '', role: UserRole.student, adConsent: false, dataConsent: false)
              );
              return InputChip(
                label: Text(user.name, style: const TextStyle(color: Colors.white, fontSize: 12)),
                backgroundColor: const Color(0xFF7B61FF).withOpacity(0.3),
                onDeleted: () => setState(() => selectedIds.remove(id)),
              );
            }),
            ActionChip(
              label: const Text('+', style: TextStyle(color: Color(0xFF7B61FF))),
                  ...
                  return Column(
                    children: [
                        ...
                        child: Column(
                          ...
                          children: [
                            Row(
                              ...
                              children: [
                                AiTranslatedText(
                                  isCc ? 'Selecionar Cc' : 'Selecionar Destinatários',
                      ...
                      Expanded(
                        ...
                        ? const Center(child: AiTranslatedText('Nenhum resultado encontrado.', style: TextStyle(color: Colors.white38)))
                        : ListView.builder(
                            ...
                            itemBuilder: (context, index) {
                              ...
                              final isSelected = listToUpdate.contains(user.id);
                              ...
                              return CheckboxListTile(
                                ...
                                secondary: CircleAvatar(
                                  ...
                                  child: Text(user.name.substring(0, 1).toUpperCase(), style: const TextStyle(color: Color(0xFF7B61FF), fontSize: 12)),
                                ),
                              );
                            },
                          ),
