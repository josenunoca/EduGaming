...
class MarketingReportScreen extends StatefulWidget {
  final List<MarketingEvent> events;
  final InstitutionModel institution;
  const MarketingReportScreen({super.key, required this.events, required this.institution});
...
class _MarketingReportScreenState extends State<MarketingReportScreen> {
  late MarketingReportDraft _draft;
  ...
  void initState() {
    super.initState();
    _draft = MarketingReportDraft.fromRawData(widget.events);
  ...
  Future<void> _generateWithAI() async {
    ...
    try {
      ...
      final result = await aiService.generateMarketingReportDraft(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const AiTranslatedText('Editor de Relatório de Marketing'),
        actions: [
          if (!_isGenerating)
            IconButton(
                    ...
                    Expanded(
                      child: CustomButton(
                        onPressed: _isExporting ? null : () async {
                          ...
                          try {
                            await PdfService.generateMarketingReportPDF(
                              widget.institution, 
                              widget.events,
                              draft: _draft,
                            );
                    ...
                    Expanded(
                      child: CustomButton(
                        onPressed: _isExporting ? null : () async {
                          ...
                          setState(() => _isExporting = true);
                          try {
                            await PdfService.generateMarketingPresentationPDF(
                              widget.institution, 
                              widget.events,
                              draft: _draft,
                            );
