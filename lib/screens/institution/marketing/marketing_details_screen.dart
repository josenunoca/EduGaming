...
class MarketingDetailsScreen extends StatefulWidget {
  final MarketingEvent event;
  final InstitutionModel institution;
  const MarketingDetailsScreen(
      {super.key, required this.event, required this.institution});

  @override
  State<MarketingDetailsScreen> createState() => _MarketingDetailsScreenState();
}

class _MarketingDetailsScreenState extends State<MarketingDetailsScreen>
    with SingleTickerProviderStateMixin {
  late TabController _tabController;
  late MarketingEvent _currentevent;
  ...
  void initState() {
    ...
    _tabController = TabController(
        length: widget.event.hasFinancialImpact ? 5 : 4, vsync: this);
    _currentevent = widget.event;
  }

  Future<void> _refreshevent() async {
    final service = context.read<FirebaseService>();
    final updated = await service.getMarketingEventById(_currentevent.id);
  ...
  Future<void> _toggleCompletion() async {
    final service = context.read<FirebaseService>();
    final newStatus = _currentevent.status == 'completed' ? 'planned' : 'completed';
    await service.updateMarketingEventStatus(_currentevent.id, newStatus);
    await _refreshevent();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          if (_tabController.index == 2 && _currentevent.media.isNotEmpty)
            IconButton(
        ...
        bottom: TabBar(
          ...
          tabs: [
            ...
            if (_currentevent.hasFinancialImpact) const Tab(text: 'Finanças'),
      ...
      body: TabBarView(
        ...
        children: [
          ...
          _buildMultimediaTab(),
          if (_currentevent.hasFinancialImpact) _buildFinancialsTab(),
          _buildWallTab(),
        ],
      ),
    );
        ...
        children: [
          ...
          GlassCard(
            child: Padding(
              ...
              child: Column(
                children: [
                  _buildDetailItem(Icons.category, 'Grupo', _currentevent.marketingGroup),
                  const Divider(color: Colors.white10),
                  _buildDetailItem(Icons.calendar_month, 'Período',
                      '${_currentevent.startDate.day}/${_currentevent.startDate.month}/${_currentevent.startDate.year} - ${_currentevent.endDate.day}/${_currentevent.endDate.month}/${_currentevent.endDate.year}'),
                  const Divider(color: Colors.white10),
                  ...
                  _buildDetailItem(Icons.person, 'Responsável',
                      _currentevent.responsibleName ?? 'Nenhum atribuído'),
          ...
          if (_currentevent.status != 'completed')
            ...
            SizedBox(
              ...
              child: OutlinedButton.icon(
                ...
                label: const AiTranslatedText('Reabrir Evento de Marketing'),
      ...
      child: Column(
        children: [
          Row(
            ...
            children: [
              AiTranslatedText(
                  '${_currentevent.participants.length} Participantes',
          ...
          Expanded(
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                final p = _currentevent.participants[index];
                return ListTile(
  ...
  Widget _buildMultimediaTab() {
    final images =
        _currentevent.media.where((m) => m.type == 'image').toList();
    final videos =
        _currentevent.media.where((m) => m.type == 'video').toList();
    final docs =
        _currentevent.media.where((m) => m.type == 'document').toList();
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 32),
          if (_currentevent.media.isEmpty)
            const Center(
                child: Padding(
  ...
  void _toggleMediaSelection(ActivityMedia media, {required bool forSocial, required bool forReport}) async {
    ...
    final newMediaList = _currentevent.media.map((m) => m.id == media.id ? updatedMedia : m).toList();
    
    final updatedevent = MarketingEvent(
      id: _currentevent.id,
      title: _currentevent.title,
      description: _currentevent.description,
      institutionId: _currentevent.institutionId,
      ...
      marketingGroup: _currentevent.marketingGroup,
      hasFinancialImpact: _currentevent.hasFinancialImpact,
      resources: _currentevent.resources,
      participants: _currentevent.participants,
      media: newMediaList,
      goals: _currentevent.goals,
      financials: _currentevent.financials,
      indicators: _currentevent.indicators,
      status: _currentevent.status,
    );
    
    setState(() => _currentevent = updatedevent);
    await context.read<FirebaseService>().saveMarketingEvent(updatedevent);
  }
  ...
  void _showSocialMediaPrep() {
    final selectedMedia = _currentevent.media
          ...
          actions: [
            if (generatedContent.isNotEmpty) ...[
              IconButton(
                onPressed: () async {
                  try {
                    await MarketingExportHelper.downloadPdf(_currentevent, platform, generatedContent);
                  } catch (e) {
              ...
              IconButton(
                onPressed: () async {
                  try {
                    await MarketingExportHelper.downloadZip(_currentevent, platform, generatedContent);
                  } catch (e) {
  ...
  void _simulateSocialImpact() async {
    ...
    final updated = MarketingEvent(
      ...
      resources: _currentevent.resources,
      participants: _currentevent.participants,
      media: _currentevent.media,
      goals: _currentevent.goals,
      financials: _currentevent.financials,
      indicators: _currentevent.indicators,
      status: _currentevent.status,
      responsibleName: _currentevent.responsibleName,
      responsibleEmail: _currentevent.responsibleEmail,
      responsiblePhone: _currentevent.responsiblePhone,
      socialMediaImpact: impact,
    );
    
    setState(() => _currentevent = updated);
    await context.read<FirebaseService>().saveMarketingEvent(updated);
        ...
        builder: (ctx) => AlertDialog(
          ...
          content: Column(
            ...
            children: [
              ...
              const SizedBox(height: 12),
              const AiTranslatedText('Estes dados foram anexados ao Evento de Marketing e constarão no relatório de marketing institucional.',
                style: TextStyle(color: Colors.white54, fontSize: 12)),
            ],
          ),
          actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              final updated = MarketingEvent(
                ...
                participants: _currentevent.participants,
                media: _currentevent.media,
                goals: [..._currentevent.goals, goal],
                financials: _currentevent.financials,
                indicators: _currentevent.indicators,
                status: _currentevent.status,
              );
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              final newGoals = _currentevent.goals.map((g) {
                if (g.id == goal.id) {
                  return ActivityGoal(
                    id: g.id,
  ...
  void _showAddResourceDialog() {
    ...
    DateTime rStartDate = _currentevent.startDate;
    DateTime rEndDate = _currentevent.endDate;
    ...
    TimeOfDay rStartTime = parseInitialTime(_currentevent.startTime);
    TimeOfDay rEndTime = parseInitialTime(_currentevent.endTime);
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () async {
                ...
                List<ActivityResource> newRes = List.from(_currentevent.resources);
                  ...
                  for (var room in selectedRooms) {
                    ...
                    while (current.isBefore(end) || current.isAtSameMomentAs(end)) {
                      final entry = TimetableEntry(
                        ...
                        institutionId: _currentevent.institutionId,
                        customActivityName: 'Evento de Marketing: ${_currentevent.title}',
                        startDate: current,
                        endDate: current,
                      );
                ...
                final updated = MarketingEvent(
                  ...
                  responsibleEmail: _currentevent.responsibleEmail,
                  responsiblePhone: _currentevent.responsiblePhone,
                  socialMediaImpact: _currentevent.socialMediaImpact,
                );
                
                await svc.saveMarketingEvent(updated);
  ...
  void _removeResource(ActivityResource r) async {
    final updated = MarketingEvent(
      ...
      marketingGroup: _currentevent.marketingGroup,
      hasFinancialImpact: _currentevent.hasFinancialImpact,
      resources: _currentevent.resources.where((item) => item.name != r.name).toList(),
      participants: _currentevent.participants,
      media: _currentevent.media,
      goals: _currentevent.goals,
      financials: _currentevent.financials,
  ...
  void _showEditDetailsDialog() {
    final descController = TextEditingController(text: _currentevent.description);
    showDialog(
      context: context,
      builder: (ctx) => AlertDialog(
        backgroundColor: const Color(0xFF1E293B),
        ...
        content: AiTextField(
          ...
          maxLines: 5,
          labelText: 'Objetivos da Evento de Marketing',
        ),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx), child: const AiTranslatedText('Cancelar')),
          ElevatedButton(
  ...
  void _confirmDeleteMedia(ActivityMedia media) async {
    ...
    if (confirm == true) {
      await context.read<FirebaseService>().removeMarketingMedia(_currentevent.id, media);
      _refreshevent();
    }
  }
  ...
  void _confirmBulkDelete() async {
    ...
    if (confirm == true) {
      final mediaToDelete = _currentevent.media.where((m) => _selectedMediaIds.contains(m.id)).toList();
      await context.read<FirebaseService>().bulkRemoveMarketingMedia(_currentevent.id, mediaToDelete);
      setState(() {
        _isSelectionMode = false;
        _selectedMediaIds.clear();
      });
      ...
      for (var file in result.files) {
        ...
        if (fileBytes != null) {
          ...
          if (downloadUrl != null) {
            ...
            await service.updateMarketingMedia(_currentevent.id, newMedia);
          }
        }
      }
      _refreshevent();
  ...
  void _showSpecializedSelector(ParticipantGroupType type) async {
    final List<String>? selectedEmails = await showDialog<List<String>>(
      ...
      builder: (ctx) => ParticipantSelectorDialog(
        ...
        initialSelectedEmails: _currentevent.participants.map((p) => p.email).toList(),
      ),
    );
    ...
    if (selectedEmails != null && selectedEmails.isNotEmpty) {
      ...
      await service.inviteGroupToMarketingEvent(
        _currentevent.id, 
        'group_${type.name}', 
        type.name, 
        selectedUsers
  ...
  void _inviteAllInstitution() async {
    ...
    await service.inviteGroupToMarketingEvent(
        _currentevent.id, 'institution', widget.institution.id, members);
    _refreshevent();
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: AiTranslatedText(
              ...
              return ListView.builder(
                ...
                itemBuilder: (context, index) => ListTile(
                  ...
                  onTap: () async {
                    ...
                    await service.inviteGroupToMarketingEvent(_currentevent.id,
  ...
  Widget _buildFinancialsTab() {
    final income = _currentevent.financials
        .where((f) => f.type == 'income')
        .fold(0.0, (sum, f) => sum + f.amount);
    final expenses = _currentevent.financials
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 12),
          if (_currentevent.financials.isEmpty)
            const Center(
                child: Padding(
          ...
          content: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 16),
                if (_currentevent.media.isNotEmpty)
                  DropdownButtonFormField<String>(
            ...
            ElevatedButton(
              onPressed: () async {
                ...
                final updated = MarketingEvent(
                  ...
                  media: _currentevent.media,
                  goals: _currentevent.goals,
                  financials: [..._currentevent.financials, record],
                  indicators: _currentevent.indicators,
                  status: _currentevent.status,
                );
                await context.read<FirebaseService>().saveMarketingEvent(updated);
  ...
  void _removeFinancialRecord(ActivityFinancialRecord record) async {
    final updated = MarketingEvent(
      ...
      goals: _currentevent.goals,
      financials:
          _currentevent.financials.where((f) => f.id != record.id).toList(),
      indicators: _currentevent.indicators,
      status: _currentevent.status,
    );
    await context.read<FirebaseService>().saveMarketingEvent(updated);
