...
class InstitutionCreditManagementScreen extends StatefulWidget {
  ...
  const InstitutionCreditManagementScreen(
      {super.key, required this.institution});

  @override
  State<InstitutionCreditManagementScreen> createState() =>
      _InstitutionCreditManagementScreenState();
}
      ...
      builder: (context) => AlertDialog(
        ...
        content: TextField(
          ...
          decoration: const InputDecoration(
              labelText: 'Créditos Máximos (Vazio = Sem limite)'),
        ),
        actions: [
          TextButton(
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              await context
                  ...
                  .setUserCreditLimit(user.id, limit);
    ...
    return showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        title: AiTranslatedText(
            'Limitar todos os ${role == UserRole.teacher ? 'Professores' : 'Alunos'}'),
        content: TextField(
          controller: controller,
          keyboardType: TextInputType.number,
          style: const TextStyle(color: Colors.white),
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              await context
                  .read<FirebaseService>()
                  .setBulkCreditLimit(widget.institution.id, role, limit);
              if (mounted) Navigator.pop(context);
            },
  ...
  Widget _buildBalanceHero(InstitutionModel inst) {
    return GlassCard(
      child: Container(
        ...
        child: Column(
          children: [
            const AiTranslatedText('Créditos Disponíveis',
              ...
              children: [
                Expanded(
                    child: _buildStatList(
                        'Alunos (Mais Ativos)', stats['top_students']!)),
                const SizedBox(width: 16),
                Expanded(
                    child: _buildStatList(
                        'Professores (Mais Ativos)', stats['top_teachers']!)),
              ],
            ),
          ],
        );
        ...
        child: Column(
          ...
          children: [
            AiTranslatedText(title,
                style: const TextStyle(
                    color: Color(0xFF00D1FF), fontWeight: FontWeight.bold)),
            const SizedBox(height: 12),
            if (users.isEmpty)
              const AiTranslatedText('Sem dados',
                ...
                itemBuilder: (context, index) {
                  ...
                  return ListTile(
                    ...
                    subtitle: AiTranslatedText(
                        'Limite: ${m.aiCreditLimit ?? 'Ilimitado'}',
                        style: const TextStyle(
      ...
      children: [
        ...
        StreamBuilder<List<CreditTransaction>>(
          ...
          builder: (context, snapshot) {
            ...
            if (txs.isEmpty)
              return const AiTranslatedText('Sem transações registadas.');
                ...
                return ListTile(
                  leading: Icon(
                    tx.type == TransactionType.recharge
                        ? Icons.add_circle
                        : Icons.remove_circle,
                    color: tx.type == TransactionType.recharge
                        ? Colors.green
                        : Colors.red,
                  ),
                  title: AiTranslatedText(tx.description,
                      style:
                          const TextStyle(color: Colors.white, fontSize: 14)),
                  subtitle: Text(
