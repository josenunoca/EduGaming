    ...
    return Scaffold(
      ...
      body: StreamBuilder<List<ExamSession>>(
        stream: context
            ...
            .streamActiveExamSessions(subject.id),
        builder: (context, snapshot) {
          ...
          sessions.sort((a, b) {
            if (a.status == b.status) {
              return b.lastHeartbeat.compareTo(a.lastHeartbeat);
            }
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              final isTimeOut =
                  DateTime.now().difference(session.lastHeartbeat).inSeconds >
                      30;
              ...
              return Padding(
                ...
                child: GlassCard(
                  child: ListTile(
                      ...
                      children: [
                        ...
                        Row(
                          children: [
                            ...
                            AiTranslatedText(
                                'Pergunta: ${session.currentQuestionIndex + 1}',
                                style: const TextStyle(
                            ...
                            const SizedBox(width: 4),
                            Text(
                                '${session.currentScore.toStringAsFixed(1)} pts',
                                style: const TextStyle(
                                    color: Colors.white70, fontSize: 12)),
                          ],
                        ),
                        ...
                        Text(
                            'Última atividade: ${session.lastHeartbeat.hour.toString().padLeft(2, '0')}:${session.lastHeartbeat.minute.toString().padLeft(2, '0')}:${session.lastHeartbeat.second.toString().padLeft(2, '0')}',
                            style: TextStyle(
                                color: isTimeOut
                                    ? Colors.redAccent
                                    : Colors.white24,
                    ...
                    trailing: session.status == 'abandoned' &&
                            !session.authorizedReentry
                        ? ElevatedButton(
                            onPressed: () =>
                                _authorizeReentry(context, session),
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: AiTranslatedText(
                'Reentrada autorizada para ${session.studentName}')));
      }
    } catch (e) {
      if (context.mounted) {
