...
void main() async {
  ...
  try {
    ...
    final geminiUrl = 'https://generativelanguage.googleapis.com/v1beta/models/gemini-flash-latest:generateContent?key=$apiKey';
    final geminiResponse = await http.post(
      Uri.parse(geminiUrl),
      headers: {'Content-Type': 'application/json'},
      body: jsonEncode({
        'contents': [{'parts': [{'text': 'Olá, estás a funcionar?'}]}]
      }),
    );
    ...
    if (geminiResponse.statusCode == 200) {
      ...
      output += 'Gemini Flash Latest Error: ${geminiResponse.statusCode} - ${geminiResponse.body}\n';
    }

    // Test TTS
    output += '\n--- TTS Test ---\n';
    final ttsUrl = 'https://texttospeech.googleapis.com/v1/text:synthesize?key=$apiKey';
